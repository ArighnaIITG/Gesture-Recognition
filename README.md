...
First, install **Keras** along with **TensorFlow**, you could simply run `pip install keras` and `pip install tensorflow `respectively if you have PyPI. You'll also need **OpenCV** or `pip install opencv-python`.

To run the application, simply execute `python3 WebCam.py` or your respective command to run python3. To train the network with different parameters, you would have to make minor modifications to [BuildTrainTestCNN.py](https://github.com/ArighnaIITG/Gesture-Recognition/blob/master/BuildTrainTestCNN.py).

I am working on adding functionality to make this process easier. For the time being, one can tweak the default values for training/testing parameters in the aforementioned file.
The values I've used are by no means perfect. I encourage you to find better configurations, and implement them on your own.

## Why did I choose this project?

I had completed the [Deep Learning](https://www.deeplearning.ai/) in Coursera and I enjoyed working on the assignements throughout that course. So, I decided to work on a side project over the summer just to see what I could build with what I had learned.

I came across an old contest on [Kaggle](https://www.kaggle.com/c/challenges-in-representation-learning-facial-expression-recognition-challenge/data) and I was intrigued, and so I started working on it.
